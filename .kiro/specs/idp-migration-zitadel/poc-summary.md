...
### 1. lockout policyが無効(重大) → 2026-09-15、k3d環境で解消・再検証済み

task7.1のセキュリティ検証で、誤パスワード15〜18回の連続試行後も直後の正しいパスワードでのログインが即座に成功し、レート制限・アカウント一時ロックアウトが一切発火しないことを確認していた。`GET /admin/v1/policies/lockout`は`maxPasswordAttempts`未設定のデフォルトポリシーを返しており、ブルートフォース対策が無効な状態のまま運用されていた。

**対応・再検証結果**: k3d `zitadel-poc`環境(Zitadel v4.12.3)で、Terraform `zitadel_default_lockout_policy`リソース(`max_password_attempts = 5`, `max_otp_attempts = 5`)をAdmin API経由(access_token=PATによるprovider認証)で適用した(REST `PUT /admin/v1/policies/lockout`は405 Method Not Allowedで直接叩けず、Terraform providerのgRPC経由でのみ更新可能なことを確認)。検証用ユーザーで誤パスワードをSession API(`POST /v2/sessions`)へ連続送信したところ、5回目まで`code:3`(Password is invalid)、6回目以降は`code:9`(`Errors.User.NotActive`)でロックアウトへ遷移し、直後の正しいパスワードでの試行も同じく`Errors.User.NotActive`で拒否されることを確認した。ブルートフォース対策が機能する状態になったことを実機で確認済み。

本番反映時は`terraform/`配下(TFCバックエンド)へ`zitadel_default_lockout_policy`リソース(org_id指定不要、instanceのデフォルトポリシー)を正式追加すること。
...
1. ~~lockout policy有効化~~ → 2026-09-15対応・再検証済み(上記「必須対応事項」参照)
