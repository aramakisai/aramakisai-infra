...
**Objective:** 実行委員として、自分自身の資格情報でログインし、ML の共有メールボックスを読み書きしたい。そうすることで、共有パスワードを使わずに ML メールへアクセスできる。

#### Acceptance Criteria
1. When ユーザーが自分自身の資格情報(Zitadel)で IMAP/Webmail にログインした場合, the メールサーバー shall 全 ML 共有メールボックスへの同等のアクセス(閲覧・既読・移動・削除・投稿)を許可する。
2. If ログインが Zitadel で認証されていない場合, then the メールサーバー shall ML 共有メールボックスへのアクセスを許可しない。
3. The メールサーバー shall ML 共有メールボックスへのアクセス制御を静的な共有パスワードに依存させない。

---

### Requirement 3: 送信者なりすまし防止と個人メール送信の廃止

**Objective:** メールサーバー運用者として、送信元(From)に使えるアドレスを ML アドレスとシステム通知用アドレスに限定し、個人メールアドレスを送信元とした送信を廃止したい。そうすることで、個人アドレスの詐称と個人メール依存を送信側でも解消できる。

#### Acceptance Criteria
1. When Zitadel で認証済みのユーザーが ML 宛アドレス(エイリアス含む)を送信元(From)として送信した場合, the メールサーバー shall 所属部署に関わらずその送信を許可する。
2. If 認証されていないクライアントが ML 宛アドレスを送信元として送信を試みた場合, then the メールサーバー shall その送信を拒否する。
3. The メールサーバー shall `noreply@aramakisai.com` を送信元とした送信を、SASL ユーザー `noreply@aramakisai.com` の場合のみ許可する。 <!-- confidential:allow -->
4. The メールサーバー shall 個人メンバーの個人メールアドレスを送信元(From)とした送信を、送信元の本人かどうかに関わらず常に拒否する。
5. The メールサーバー shall 送信元(From)として使用可能なアドレスを ML 宛アドレスの集合と `noreply@aramakisai.com` のみに制限する。 <!-- confidential:allow -->
