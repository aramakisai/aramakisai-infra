...
- GitOps (ArgoCD) + ESO (Infisical) + CNPG (B2 WAL) + VolSync (B2 restic) で状態を外部に保持しているため、ノードを作り直すだけで復旧できる
...
#### REQ-03-2: Stalwart TLS 証明書タイミング問題

- **背景**: DR 後に `cert-manager` の `mail-tls` Certificate が ArgoCD sync タイミングにより未適用になる場合がある
- **対応**: stalwart-0 が `ContainerCreating` のまま 2分以上停止している場合、`mail-tls` の存在を確認し、存在しない場合は手動 apply する
...
#### REQ-03-4: Directus 空 DB 通知

- **背景**: Directus は WAL アーカイブ未設定のため、DR 後は常に空 DB で起動する
- **対応**: recovery.sh 完了後、GitHub Actions の `::warning::` でコンテンツ再投入が必要な旨を出力する（自動投入はスコープ外）
...
| アラート | ログソース | 条件 |
|---------|-----------|------|
| SMTP ブルートフォース | Stalwart ログ | 1分間に同一 IP から認証失敗 10件以上 |
...
| Stalwart (メール) | 最大2時間 | 30分 | VolSync スナップショット間隔 |
| Directus | ∞（空DB）| 30分 | WAL 未設定・コンテンツ再投入必要 |
