...
# タスク 5.10: SSH トンネル律速疑義の切り分け（ノード上直接実行、2026-09-21 20:52頃〜20:55頃 JST）

タスク 5.9 のエラー内訳が `read tcp 127.0.0.1:18090: read: connection reset by peer`（測定 5: 2194 件、測定 6: 715 件）であり、これが SSH ポートフォワードのローカル終端で発生するシグネチャであったこと、サーバー側（CMS Pod CPU 58%、ノード CPU 71%、`directus-db-1` 186m）がいずれも余裕を残していたことから、「SSH トンネル自体が毎秒 100 本規模の接続開閉で天井になっていたのではないか」という疑義が生じた。この疑義を切り分けるため、SSH を経路から外し、`prod-node-1` 上で k6 を直接実行して CMS の ClusterIP (`10.43.111.239:80`) を叩く再測定を行った。

## 実行方法

- k6 バイナリ: GitHub Releases から `k6-v2.2.0-linux-amd64.tar.gz` を `curl` で取得し `/tmp` に展開（`apt`/`apk` 等のパッケージマネージャは不使用）。ローカルで使用している Docker イメージ `grafana/k6:latest` の `k6 version` 出力（`k6 v2.2.0`）と完全一致するバージョンを選定した。
- シナリオ: `git show feat/festival-peak-scaleout:scripts/load-test/breakpoint.js` を `scp` で `/tmp/breakpoint.js` としてノードへ転送。変更なしでそのまま使用。
- 実行コマンド（ノード上）:
  ```
  BASE_URL=http://10.43.111.239 TARGET_PATH='/api/globals/festival_meta?depth=1' \
  START_RATE=50 MAX_RATE=200 RAMP_DURATION=2m PRE_ALLOCATED_VUS=200 MAX_VUS=1500 \
  ./k6-bin run breakpoint.js
  ```
  タスク 5.9 測定 6 と同一パラメータ（比較のため）。SSH は上記コマンドの投入のみに使用し、負荷トラフィックは経由していない。
- 監視: ローカル PC から `make kubectl` で CMS Pod・`directus-db-1`・ノードの `top` をポーリング（間隔は `infisical run` のオーバーヘッドにより実測 21〜22 秒）。加えて `prod-node-1` 上で `top -bn1 | grep k6-bin` により k6 プロセス自体の CPU 使用率を別途サンプリングし、CMS の値と区別できるようにした。

## 結果

- `abortOnFail` 発動: 開始 1 分 57.9 秒後（ランプ終了間際、目標レート ≈197 req/s 付近）
- `error_rate` / `http_req_failed`: 11.28%（1408/12481）
- `http_req_duration`: avg=2.79s, min=0s, med=993.87ms, p90=8.34s, p95=9.99s, max=10.01s（k6 側タイムアウト値）
- 実測スループット: k6 報告値 **105.81 req/s**（`http_reqs` レート。12481 リクエスト / 117.9s）
- エラー種別内訳（測定 5・6 と異なる分類）: `dial tcp 10.43.111.239:80: connect: connection refused` 704 件、`request timeout`（k6 側 10 秒タイムアウト）704 件。**`connection reset by peer` は 0 件**（測定 5・6 で支配的だった SSH トンネルのローカル終端シグネチャが、経路から SSH を外したことで消滅した）。

### CMS Pod CPU/メモリ・ノード・k6 プロセスの実測（21〜22 秒間隔サンプリング）

| 時刻 | CMS Pod CPU | CMS Pod MEM | directus-db-1 CPU | ノード CPU | k6-bin プロセス CPU |
|---|---|---|---|---|---|
| 20:53:19（アイドル） | 10m | 104Mi | 10m | 1008m/26% | 15.4% |
| 20:53:41 | 12m | 105Mi | 107m | 2029m/53% | 7.1% |
| 20:54:03 | 887m | 126Mi | 134m | 2139m/56% | 13.3% |
| 20:54:24 | 1017m | 125Mi | 151m | 2365m/62% | 23.1% |
| 20:54:46 | 1145m | 139Mi | 177m | 2412m/63% | 28.6% |
| 20:55:08（abortOnFail 直前） | 1214m | 181Mi | 177m | 2517m/66% | 28.6% |
| 20:55:29（abortOnFail 直後） | 1206m | 252Mi | 183m | 2500m/65% | — |

- **CMS Pod CPU のピーク実測値: 1214m**（`limits.cpu: 2000m` に対し 60.7%。タスク 5.9 測定 6 の 1166m/58% とほぼ同水準で、制限値付近への張り付きは今回も観測されなかった）
- **CMS Pod メモリのピーク実測値: 252Mi**（`limits.memory: 512Mi` に対し 49.2%。ただしこれは abortOnFail 直後のサンプルで、負荷停止後もしばらく増加が続いており GC 遅延を含む可能性がある。OOMKilled なし）
- `directus-db-1` CPU のピーク実測値: 183m（タスク 5.9 測定 6 の 186m と同水準。DB が律速でないことは変わらず）
- k6-bin プロセス自体の CPU 使用率: 最大 28.6%（1 コア分の約 3 割弱）。ノードは 4 vCPU のため、k6 自身がノード全体の CPU 使用率を最大でも 1 コア相当以下押し上げている。CMS Pod の CPU（最大 1214m）と比べれば小さいが、無視できる量ではない

## タスク 5.9（SSH トンネル経由）との比較

| | タスク 5.9 測定 5・6（SSH トンネル経由） | タスク 5.10（ノード上直接実行） |
|---|---|---|
| 目標レート到達点 | 約 177-185 req/s | 約 197 req/s |
| 実測スループット | 約 86.5-100.5 req/s | **約 105.8 req/s** |
| CMS Pod CPU ピーク | 1166m（limit 2000m の 58%） | 1214m（limit 2000m の 60.7%） |
| 主要エラー種別 | `connection reset by peer`（SSH トンネルのローカル終端）が支配的 | `connection refused`（サーバー側 TCP accept 拒否）と `request timeout` が同数（704/704） |

実測スループットは 86.5-100.5 → 105.8 req/s と、**約 5〜22% の上昇に留まった**。SSH トンネルが律速の主因であれば、経路を外すことで数倍規模の改善が見込まれるはずだが、そのような大きな変化は観測されなかった。CMS Pod CPU ピークも 58%→60.7% とほぼ同水準であり、CPU limit にはやはり到達していない。

## 律速箇所の再判定

**タスク 5.9 の「アプリ（Node.js/Payload）側の処理能力が律速」という判定は訂正を要しない。今回の測定で裏付けられた。** SSH トンネルは律速の主因ではなかった。

根拠:

1. SSH を経路から外しても実測スループットは 5〜22% しか改善せず、SSH トンネルが天井であったなら生じるはずの大幅な改善（数倍規模）が見られなかった。
2. CMS Pod CPU ピークは経路を変えても 58%→60.7% とほぼ同水準で、`limits.cpu: 2000m` に対する余裕度もほぼ変わらない。CPU 律速でないことも変わらず。
3. エラーの発生箇所が質的に変化した点がむしろ重要な追加根拠になる。SSH トンネル経由では `connection reset by peer`（k6 実行元のローカル終端、クライアント側アーティファクト）が支配的だったが、SSH を排したこの測定では代わりに `connection refused`（TCP dial 自体の拒否）が現れた。SSH という仲介プロセスが存在しない直結経路で `connection refused` が発生するということは、**CMS（Node.js）プロセス自身が新規接続の accept を追いつかせられていない**ことを直接示すサーバー側の兆候であり、SSH アーティファクトでは説明できない。前回の「SSH 側が天井になった可能性」という疑義に対し、この観測はむしろ逆の証拠（サーバー側が接続を捌ききれていない）を示している。

以上より、前回測定（タスク 5.9）の律速判定「CPU limit ではなくアプリケーション側の処理能力が新たな律速」は誤りではなく、SSH トンネル経由という測定方法によって歪められたものでもなかったと結論する。

## 測定値への影響（k6 のノード上実行による CPU 競合）

k6 を CMS と同一ノード上で実行したため、k6-bin プロセス自体がノード CPU を最大 28.6%（1 コア未満）消費しており、ノード全体の CPU リソースの一部を CMS・`directus-db-1` と奪い合っている。この競合分だけ、本測定のスループット・CMS Pod CPU の値は実際の「オリジン直・専用クライアントから打った場合」より保守側（低め）にぶれている可能性がある。ただし CMS Pod 自体は cgroup で CPU リソースが分離されており（`limits.cpu: 2000m`）、ノード全体の CPU 使用率がボトルネックになっていた形跡（ノード CPU ピーク 66%、上限に対し余裕あり）もないため、この競合が判定を覆すほどの影響を与えたとは考えにくい。

## 測定後の復帰確認

- ノード直（ClusterIP 直、SSH 越しに確認）: `curl http://10.43.111.239/api/globals/festival_meta?depth=1` → `200`
- Cloudflare 経由: `curl https://cms.aramakisai.com/api/globals/festival_meta?depth=1` → `200`
- `make kubectl ARGS="get pods -n prod -o wide"`: 全 Pod `Running`。CMS `RESTARTS 0`（測定前後で変化なし）。`directus-db-1` の `RESTARTS 1 (21d ago)` は測定以前からの既存値で本測定による増加ではない
- ノード `/tmp` に配置した k6 バイナリ・シナリオファイル・生成物はすべて削除済み。`k6-bin` プロセスの残存なし

## 未確認・未解決の点

- `connection refused` が Node.js の TCP listen backlog（`server.listen()` の `backlog` パラメータ既定値）由来か、OS の `somaxconn`／ephemeral port 枯渇由来か、あるいはアプリケーションが明示的に接続を絞っているのかは、本測定ではプロセス内部・カーネルパラメータの調査を行っておらず特定できていない。
- k6 プロセスとノード CPU の競合を完全に排除した測定（k6 を別ノードまたはクラスタ外から ClusterIP へ到達させる等）は行っていない。ただし前節の通り、この競合が本タスクの判定を覆す規模とは考えにくい。
- タスク 5.9 と同じく、CPU 使用率が 2000m に到達せず 1214m で頭打ちになった直接原因（イベントループ、DB コネクションプールサイズ、keep-alive 等）は未特定のまま。
