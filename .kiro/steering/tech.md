...
構成の一貫性と引き継ぎ可能性を保つための全体制約。逸脱する場合は理由を本ドキュメントに残す。

- **`kubectl port-forward` を経路にしない**: 一時的なデバッグ用途に限る。運用手順や IaC の前提に組み込むと、特定の端末でしか再現できない手順が恒久化する。
- **NodePort を使わない**: 例外はメールサーバー関連のみ。ノードの全インターフェースに bind されるため、公開範囲がファイアウォールの allow-list 設定だけに依存する状態になる。
- **IP アドレスを直接指定しない**: ノードのパブリック IP・Tailscale IP を含む。例外はメールサーバー関連のみ。接続先はホスト名で表現し、アドレス変更時の追従点を DNS に一本化する。
- **サブドメインを冗長に増やさない**: 既存のホスト名で目的を達成できないか先に検討する。ホスト名が増えるほど DNS レコード・証明書・Tunnel ルート・Access ポリシーの管理点が比例して増える。
- **ArgoCD Application を不用意に増やさない**: 既存 Application のマニフェストに収まらないか先に検討する。Application 単位で sync ポリシー・sync-wave・prune 挙動が独立するため、数が増えるほど反映順序の把握が難しくなる。
- **Terraform 外でリソースを管理しない**: 手動スクリプトや REST API の直接呼び出しによる作成・変更を行わない。Terraform provider に該当リソースが存在せず SaaS の管理画面でしか設定できない項目は、その事実と設定内容を本ドキュメントに明記する。
