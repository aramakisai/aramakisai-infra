...
**`vaultwarden-rbac-sync`連携時の注意**: Collection名はOrganization鍵でクライアント側暗号化されるため、Vaultwarden APIから人間可読名としては取得できない（実機検証済み、`research.md`参照）。`mapping.json`にはCollection名ではなく**Collection ID (UUID)**を記述する。IDはWeb Vault → 対象 Organization → Collections → 対象Collectionを開いた際のURL/詳細画面で確認できる（Org Owner/Adminのブラウザ操作が必要）。
