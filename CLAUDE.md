...
保護対象:
  ...
  webmail.aramakisai.com   CF Access Application (Snappymail webmail)
...
非保護 (自前認証あり):
  ...
  mail-admin.aramakisai.com  Stalwart 独自のログインで保護
                             CF Access は設定しない (Stalwart の OIDC issuer が
                             mail.aramakisai.com のため Access と組み合わせ困難)
```
...
* **Hetzner FW でポート 443 は開放しない** (Stalwart 管理は Tailscale + kubectl port-forward のみ)
  - Stalwart admin UI: `kubectl port-forward -n prod pod/stalwart-0 8443:443` → https://localhost:8443/admin/
...
Webmail ブラウザアクセス
  → webmail.aramakisai.com → CF Access (Authentik) → CF Tunnel → Snappymail → Stalwart IMAP

管理 UI
  → mail-admin.aramakisai.com → CF Tunnel → stalwart-admin:443 (Stalwart 独自認証)
  → または kubectl port-forward (Tailscale 経由, 管理者のみ)
```

**port 443 はファイアウォールで閉鎖。** Stalwart の OIDC issuer が `mail.aramakisai.com` であり
CF Tunnel と組み合わせが困難なため。JMAP/webmail は Snappymail 経由で提供。
